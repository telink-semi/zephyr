...
static void gpio_b9x_irq_handler(const struct device *dev)
{
	...
	uint8_t irq = GET_IRQ_NUM(dev);
	uint8_t status = gpio_b9x_irq_en_get(dev);
					    ...
					    enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	case GPIO_INT_MODE_LEVEL:
		if (trig == GPIO_INT_TRIG_HIGH) { /* GPIO interrupt High level */
			gpio_b9x_irq_set(dev, pin, INTR_HIGH_LEVEL);
		} else if (trig == GPIO_INT_TRIG_LOW) { /* GPIO interrupt Low level */
			gpio_b9x_irq_set(dev, pin, INTR_LOW_LEVEL);
		} else {
			ret_status = -ENOTSUP;
		}
		break;
...
#define GPIO_B9X_INIT(n)                                                                           \
	...
	static struct gpio_b9x_data gpio_b9x_data_##n;                                             \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, gpio_b9x_init, NULL, &gpio_b9x_data_##n, &gpio_b9x_config_##n,    \
			      PRE_KERNEL_1, CONFIG_GPIO_INIT_PRIORITY, &gpio_b9x_driver_api);
