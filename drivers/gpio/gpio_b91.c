...
static void gpio_b91_irq_handler(const struct device *dev)
{
    ...
    gpio_port_value_t changed_pins0      = (cfg->pin_irq_state->pin_last_value ^ current_pins) & (~current_pins);
    gpio_port_value_t changed_pins1      = (cfg->pin_irq_state->pin_last_value ^ current_pins) & current_pins;
    ...
    gpio_port_value_t fired_irqs_both    = (changed_pins0 | changed_pins1) & cfg->pin_irq_state->irq_en_both;
