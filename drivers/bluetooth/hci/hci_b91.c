...
static struct net_buf *bt_b91_evt_recv(uint8_t *data, size_t remaining)
{
	bool discardable;
...
static struct net_buf *bt_b91_acl_recv(uint8_t *data, size_t remaining)
{
...
static struct net_buf *bt_b91_iso_recv(uint8_t *data, size_t remaining)
{
...
static void hci_b91_host_rcv_pkt(uint8_t *data, uint16_t len)
{
	...
	case HCI_EVT:
		buf = bt_b91_evt_recv(data, remaining);
		break;

	case HCI_ACL:
		buf = bt_b91_acl_recv(data, remaining);
		break;

	case HCI_SCO:
		buf = bt_b91_iso_recv(data, remaining);
		break;
...
static b91_bt_host_callback_t vhci_host_cb = {
	hci_b91_controller_rcv_pkt_ready,
	hci_b91_host_rcv_pkt
};
