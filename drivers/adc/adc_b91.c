...
/* Get ADC value */
signed short adc_b91_get_code(void)
{
	...
	if ((adc_code & BIT(SIGN_BIT_POSITION)) && !m_data.differential) {
		adc_code = 0;
	}
...
{
	m_data.repeat_buffer = m_data.buffer;

	adc_power_on();

	k_sem_give(&m_data.acq_sem);
}
...
{
	if (repeat_sampling) {
		m_data.buffer = m_data.repeat_buffer;
	}
}
...
static int adc_b91_adc_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	case 14:
		...
		m_data.resolution_offset = 0;
		break;
	case 12:
		adc_set_resolution(ADC_RES12);
		m_data.resolution_offset = 2;
		break;
	case 10:
		adc_set_resolution(ADC_RES10);
		m_data.resolution_offset = 4;
		break;
	case 8:
		adc_set_resolution(ADC_RES8);
		m_data.resolution_offset = 6;
		break;
	default:
...
static void adc_b91_acquisition_thread(const struct device *dev)
{
	...
	while (true) {
		...
		k_sem_take(&m_data.acq_sem, K_FOREVER);
		...
		/* Perform read */
		adc_code = (adc_b91_get_code() >> m_data.resolution_offset);
		adc_code = adc_code << (m_data.differential ? 0 : 1);
		*m_data.buffer++ = adc_code;
...
{
	k_sem_init(&m_data.acq_sem, 0, 1);

	k_thread_create(&m_data.thread, m_data.stack,
			...
			const struct adc_sequence *sequence)
{
	...
	adc_context_release(&m_data.ctx, status);
